...
static int window_lensing(double chi,ccl_cosmology *cosmo,SplPar *spl_pz,double chi_max,double *win)
{
  ...
  gslstatus=gsl_integration_qag(&F,chi,chi_max,0,1E-3,1000,GSL_INTEG_GAUSS41,w,&result,&eresult);
  *win=result;
...
static double f_dens(double a,double k,ccl_cosmology *cosmo,CCL_ClTracer *clt, int * status)
{
  double z=1./a-1;
  ...
  if(cosmo->params.fNL!=0) {
    double tk=ccl_matter_transfer(cosmo,k,status)*ccl_growth_factor_unnorm(cosmo,a,status);
    double kh=k*CLIGHT_HMPC;
    double prefac=3*cosmo->params.fNL*cosmo->params.Omega_m*cosmo->params.h*cosmo->params.h*1.686;
    bz+=prefac*(bz-1)/(kh*kh*tk);
  }
  return pz*bz*h;
}
			  ...
			  ccl_cosmology *cosmo,CCL_ClWorkspace *w,CCL_ClTracer *clt, int * status)
{
  ...
  if(l>w->l_limber) {
    ...
    if(chi0<=clt->chimax) {
      ...
      double f_all=f_dens(a0,k,cosmo,clt,status)*jl0;
      if(clt->has_rsd) {
	double x1=(l+1.5);
	double chi1=x1/k;
	if(chi1<=clt->chimax) {
	...
	double f_all=f_dens(a,k,cosmo,clt,status)*jl;
	if(clt->has_rsd) {
	  double ddjl,x=k*chi;
	  if(x<1E-10) {
			 ...
			 int *status)
{
  ...
  double *lkarr;
  double lknew=lkmin;
  double k_period=2*M_PI/chimax/2;
  double dklin=0.45;
  ik=0;
  while(lknew<=lkmax) {
...
static void compute_transfer(CCL_ClTracer *clt,ccl_cosmology *cosmo,CCL_ClWorkspace *w,int *status)
{
  ...
  clt->n_ls=w->n_ls;
  clt->n_k=(int *)calloc(clt->n_ls,sizeof(int));
  if(clt->n_k==NULL) {
    *status=CCL_ERROR_MEMORY;
...
#pragma omp parallel default(none)\
  shared(clt,cosmo,w,status,zmin,chimin,chimax)
  {
    int il;
...
#pragma omp for schedule(dynamic)
  ...
  if(*status) {
    int ill;
    for(ill=0;ill<clt->n_ls;ill++) {
      if(clt->n_k[ill]>0)
	spline_free(clt->spl_transfer[ill]);
    }
    free(clt->spl_transfer);
    free(clt->n_k);
  }
