...
#define K_LARGE 1E-4
double ccl_matter_transfer(ccl_cosmology *cosmo,double k,int *status)
{
  double pkL=ccl_linear_matter_power(cosmo,K_LARGE,1.,status);
  double pk =ccl_linear_matter_power(cosmo,k,1.,status);
  return sqrt(pow(K_LARGE/k,cosmo->params.n_s)*pk/pkL);
}
